...
int comparePriority(Message *m1, Message *m2) {
    if (m1->Tm < m2->Tm) {
        ...
        if (m1->process_num < m2->process_num) {
            return -1;
        } else if (m1->process_num > m2->process_num) {
            return 1;
        } else {
            return 0;
        }
...
void printQueue(PriorityQueue *pq) {
    ...
    for (int i = 0; i < pq->size; i++) {
        printf("[%d] Tm: %ld, Process: %d, Type: %ld, Text: %s\n", 
               i, pq->messages[i].Tm, pq->messages[i].process_num, 
               pq->messages[i].mtype, pq->messages[i].mtext);
    }
    printf("\n");
}